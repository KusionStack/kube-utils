...
func controlledHistories(c client.Client, owner client.Object, labelSelector *metav1.LabelSelector) ([]*apps.ControllerRevision, error) {
	...
	writer := refmanager.NewOwnerRefWriter(c)
	matcher, err := refmanager.LabelSelectorAsMatch(labelSelector)
	if err != nil {
		return nil, fmt.Errorf("fail to create labelSelector matcher: %s", err.Error())
	}

	refManager := refmanager.NewObjectControllerRefManager(writer, owner, owner.GetObjectKind().GroupVersionKind(), matcher)

	mts := make([]metav1.Object, len(histories.Items))
	for i := range histories.Items {
		mts[i] = &histories.Items[i]
	}
	claims, err := refManager.ClaimAllOf(context.TODO(), mts)
	if err != nil {
		return nil, err
	}
...
func (rm *RevisionManager) ConstructRevisions(set client.Object, dryRun bool) (*apps.ControllerRevision, *apps.ControllerRevision, []*apps.ControllerRevision, *int32, bool, error) {
	...
	revisions, err := controlledHistories(rm.Client, set, rm.ownerGetter.GetSelector(set))
	if err != nil {
